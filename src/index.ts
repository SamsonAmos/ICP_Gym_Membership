...
export function fetchAllGyms(): Result<Vec<Gym>, string> {
  try {
    const gyms = gymStorage.values();
    if (gyms.length === 0) {
      return Result.Err("No gyms found.");
    }
    return Result.Ok(gyms);
  } catch (error) {
    return Result.Err(`Failed to fetch gyms: ${error}`);
  }
}
...
export function fetchGymById(id: string): Result<Gym, string> {
  return match(gymStorage.get(id), {
    ...
    None: () =>
      Result.Err<Gym, string>(`a gym with this id=${id} is not found`),
  });
}
...
$update;
export function addGymService(
  id: string,
  payload: GymServicePayload
): Result<Gym, string> {
  ...
  const gym = match(gymStorage.get(id), {
    ...
    None: () =>
      Result.Err<Gym, string>("Unable to carry out the following function"),
  });
...
export function registerForAgym(id: string): Result<Gym, string> {
  const gym = match(gymStorage.get(id), {
    ...
    None: () => Result.Err<Gym, string>(`Gym with id ${id} not found.`),
  });
...
export function deleteAGymById(id: string): Result<Gym, string> {
  return match(gymStorage.get(id), {
    ...
    None: () =>
      Result.Err<Gym, string>(
        `couldn't delete the gym with this id=${id}. gym not found.`
      ),
  });
}
